...
exports.createOrder=catchAsyncErrors(async(req,res,next)=>{
    ...
    const order= await Order.create({        orderItems,shippingInfo,itemsPrice,taxPrice,shippingPrice,totalPrice,paymentInfo,
          paidAt:Date.now(),
          user:req.params._id
    })
    res.status(200).json({
...
exports.getSingleOrders=catchAsyncErrors(async(req,res,next)=>{ 
    const order=await Order.findById(req.params.id).populate('user','name email')
        ...
        if(!order){
            return next(new ErrorHandler('order do not find by this ID',404))
        }
        res.status(200).json({
            success:true,
            order
...
async function updateStock(id,quantity){
    ...
    product.stock=product.stoke - quantity
